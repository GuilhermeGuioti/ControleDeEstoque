...
const SalesPage = ({
  ...
  const cartTotal = useMemo(
    () => cart.reduce((t, i) => t + i.preco * i.quantidade, 0),
    [cart],
  );
  const cartItemsCount = useMemo(
    () => cart.reduce((t, i) => t + i.quantidade, 0),
    [cart],
  );
  ...
  const handleCheckout = async () => {
    ...
    if (cart.length === 0) {
      setCheckoutError(
        "Adicione pelo menos um item ao carrinho para registrar a venda.",
      );
      return;
    }
      ...
      <Grid container spacing={2.5} sx={{ flex: 1, minHeight: 0 }}>
          ...
          >
            ...
            >
              <TextField
                fullWidth
                placeholder={
                  activeTab === 0 ? "Buscar produto..." : "Buscar serviço..."
                }
                value={searchTerm}
        ...
        >
          ...
          >
            ...
            {/* Cart items */}
            <Box
              sx={{ flex: 1, overflowY: "auto", px: 2, py: 1.5, minHeight: 0 }}
            >
              {cart.length === 0 ? (
                <Box sx={{ textAlign: "center", py: 5 }}>
                  ...
                  >
                    <ShoppingCartIcon
                      sx={{ color: "primary.main", fontSize: 24, opacity: 0.5 }}
                    />
                  </Box>
                  ...
                  {cart.map((item, idx) => (
                    <Box
                      ...
                      sx={{
                        py: 1.5,
                        borderBottom:
                          idx < cart.length - 1 ? "1px solid" : "none",
                        borderColor: "divider",
                      }}
                    >
                      <Stack
